...
const Home: React.FC = () => {
  ...
  const handleAllocate = async () => {
    if (selectedStudents.length === 0 || selectedFaculties.length === 0) {
      alert("Please select students and faculty before allocating.");
      return;
    }
                    ...
                    {faculties.map((faculty) => (
                      <tr key={faculty.fname} className="bg-white">
                        ...
                        <td className="border p-2 text-center">
                          <input
                            type="checkbox"
                            checked={selectedFaculties.includes(faculty.fname)}
                            onChange={() => handleFacultySelection(faculty.fname)}
                          />
                        </td>
                      </tr>
                    ))}
